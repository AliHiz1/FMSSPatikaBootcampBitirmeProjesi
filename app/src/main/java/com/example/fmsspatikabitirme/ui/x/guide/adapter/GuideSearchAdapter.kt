...
import com.example.fmsspatikabitirme.ui.x.guide.GuideSearchFragmentDirections
...
class GuideSearchAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private lateinit var binding: ViewDataBinding
    private var travelModel =  TravelModel()
    private var searchArray = ArrayList<TravelAppModel>()

    class GuideSearchViewHolder(private val binding: ViewDataBinding) :
        RecyclerView.ViewHolder(binding.root)
    {
        fun SearchListBind(travelAppModel: TravelAppModel){
            ...
            binding.searchImgrow2.setOnClickListener { view ->
                val action = GuideSearchFragmentDirections.actionGuideSearchFragmentToDetailsFragment(travelAppModel)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return GuideSearchViewHolder(binding)
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as GuideSearchViewHolder).SearchListBind(searchArray[position])
